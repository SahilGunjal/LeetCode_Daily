...
Approach: Just go through the linked list once and put visited nodes in dict accordingly.
O
Tc: O(n)
Sc: O(n)
"""
...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        while ptrHead:
            if newHead is None:
                newHead = Node(ptrHead.val)
                my_dict[ptrHead] = newHead
                newPtr = newHead

                if ptrHead.random is None:
                    newPtr.random = None
                else:
                    if ptrHead.random in my_dict:
                        newPtr.random = my_dict[ptrHead.random]
                    else:
                        newPtr.random = Node(ptrHead.random.val)
                        my_dict[ptrHead.random] = newPtr.random


            else:
                if ptrHead in my_dict:
                    newPtr.next = my_dict[ptrHead]
                    newPtr = newPtr.next

                else:
                    newPtr.next = Node(ptrHead.val)
                    my_dict[ptrHead] = newPtr.next
                    newPtr = newPtr.next
